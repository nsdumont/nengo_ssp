...
class SpatialSemanticPointer(SemanticPointer):
    ...
    def _get_binary_name(self, other, op, swap=False):
        if isinstance(other, SpatialSemanticPointer):
    ...
    def _bind(self, other, swap=False):
        ...
        if swap:
            a, b = b, a
        if type(other)==SemanticPointer:
            return SemanticPointer(
            data=other.algebra.bind(a, b), vocab=vocab, algebra=other.algebra,
            name=self._get_binary_name(other_pointer, "*", swap))
        else:
            return SpatialSemanticPointer(
    ...
    def reinterpret(self, vocab):
        ...
        """
        return SpatialSemanticPointer(self.v, vocab=vocab, name=self.name)
